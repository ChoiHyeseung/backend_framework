...
import net.skhu.repository.ProductRespository;

@Controller
@RequestMapping("product")
public class ProductController {

    @Autowired ProductRespository productRepository;
    @Autowired CategoryRepository categoryRepository;

    @RequestMapping("list")
    public String ist(Model model, Pagination pagination) {
    ...
    public String list2(Model model,@RequestParam("id") int id) {
        List<Product> products = productRepository.findByJumoonCustomerId(id);
        model.addAttribute("products", products);
        model.addAttribute("categories" , categoryRepository.findAll());
